...
bool Logger::createPort(const std::string& portname, const std::string& typestr)
{
    ...
    ports()->addEventPort(ip);

    RTT::DataSourceBase::shared_ptr orig = ip->getDataSource();
    if (! orig->getTypeInfo()->getProtocol(ORO_UNTYPED_PROTOCOL_ID))
    {
        log(Error) << "cannot report port " << portname << " from type " << typestr << " as its toolkit has not been generated by Orogen" << endlog();
        return false;
    }
